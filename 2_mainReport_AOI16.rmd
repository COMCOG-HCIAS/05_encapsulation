...
}{Henri Funk \\ Helen Alber \\Marie Scherzer\\Lona Koers
}
...
\newpage
```{r model estimation, results = 'asis', cache = TRUE}
...
\newpage
### Second Model (for AOI 1)
```{r model estimation 2, results = 'asis'}
dat <- dat %>%
...
dat_mod2$var1 <- factor(dat_mod2$var1)
dat_mod2$var2 <- factor(dat_mod2$var2)
```

```{r}
plots <- function(x_var, y_var, col_gr_var, with_errorbar = FALSE) {
  p <- ggplot(dat_mod2, 
  ...
  if(deparse(substitute(x_var)) == "dat_mod2$var1") {
    p <- p + labs(x = "variable 1", color = "variable 2")
  } else {
    p <- p + labs(x = "variable 2", color = "variable 1")
  }
  if(deparse(substitute(y_var)) == "dat_mod2$TRT.L") {
    p <- p + labs(y = "TRT per letter")
  } else if(deparse(substitute(y_var)) == "dat_mod2$FRT.L") {
...
# PLOT - TRT
plots(dat_mod2$var1, dat_mod2$TRT.L, dat_mod2$var2)
cat("\n\n")
plots(dat_mod2$var1, dat_mod2$TRT.L, dat_mod2$var2, with_errorbar = TRUE)
cat("\n\n")
plots(dat_mod2$var2, dat_mod2$TRT.L, dat_mod2$var1)
cat("\n\n")
plots(dat_mod2$var2, dat_mod2$TRT.L, dat_mod2$var1, with_errorbar = TRUE)
cat("\n\n")
# PLOT - FRT
plots(dat_mod2$var1, dat_mod2$FRT.L, dat_mod2$var2)
cat("\n\n")
plots(dat_mod2$var1, dat_mod2$FRT.L, dat_mod2$var2, with_errorbar = TRUE)
cat("\n\n")
plots(dat_mod2$var2, dat_mod2$FRT.L, dat_mod2$var1)
cat("\n\n")
plots(dat_mod2$var2, dat_mod2$FRT.L, dat_mod2$var1, with_errorbar = TRUE)
cat("\n\n")
# PLOT - RRT
plots(dat_mod2$var1, dat_mod2$RRT.L, dat_mod2$var2)
cat("\n\n")
plots(dat_mod2$var1, dat_mod2$RRT.L, dat_mod2$var2, with_errorbar = TRUE)
cat("\n\n")
plots(dat_mod2$var2, dat_mod2$RRT.L, dat_mod2$var1)
cat("\n\n")
plots(dat_mod2$var2, dat_mod2$RRT.L, dat_mod2$var1, with_errorbar = TRUE)
```

```{r}
#### MODELS

mod_TRT_int <- gam(data = dat_mod2,
               formula = TRT.L ~ var1 * var2 +
                 s(Participant, bs = "re") + s(Topic, bs = "re"))
mod_FRT_int <- gam(data = dat_mod2,
               formula = FRT.L ~ var1 * var2 +
                 s(Participant, bs = "re") + s(Topic, bs = "re"))
mod_RRT_int <- gam(data = dat_mod2,
               formula = RRT.L ~ var1 * var2 +
                 s(Participant, bs = "re") + s(Topic, bs = "re"))

###### TRY OUT
# mod_TRT <- gam(data = dat_mod2,
#                formula = TRT.L ~ condition +
#                  s(Participant, bs = "re") + s(Topic, bs = "re"))
# mod_FRT <- gam(data = dat_mod2,
#                formula = FRT.L ~ condition +
#                  s(Participant, bs = "re") + s(Topic, bs = "re"))
# mod_RRT <- gam(data = dat_mod2,
#                formula = RRT.L ~ condition +
#                  s(Participant, bs = "re") + s(Topic, bs = "re"))
...
```{r model results 2, results='asis'}
# print(summary(mod_TRT_int))
# print(summary(mod_FRT_int))
# print(summary(mod_RRT_int))
# print(summary(mod_TRT))
# print(summary(mod_FRT))
# print(summary(mod_RRT))

names_res <- c("Estimates", "Std.Error", "p.value")

res_mod_TRT_int <- data.frame(cbind(round(summary(mod_TRT_int)$p.table[, c(1, 2, 4)], 4)))
res_mod_TRT_int <- rownames_to_column(res_mod_TRT_int, var = "TRT")
colnames(res_mod_TRT_int) <- c("TRT per Letter", names_res)
res_mod_FRT_int <- data.frame(cbind(round(summary(mod_FRT_int)$p.table[, c(1, 2, 4)], 4)))
res_mod_FRT_int <- rownames_to_column(res_mod_FRT_int, var = "FRT")
colnames(res_mod_FRT_int) <-  c("FRT per Letter", names_res)
res_mod_RRT_int <- data.frame(cbind(round(summary(mod_RRT_int)$p.table[, c(1, 2, 4)], 4)))
res_mod_RRT_int <- rownames_to_column(res_mod_RRT_int, var = "RRT")
colnames(res_mod_RRT_int) <- c("RRT per Letter", names_res)
...
kable(res_mod_TRT_int)
kable(res_mod_FRT_int)
kable(res_mod_RRT_int)
...
For the TRT model: `r rand_eff_message(mod_TRT_int)`

For the FRT model: `r rand_eff_message(mod_FRT_int)`

For the RRT model: `r rand_eff_message(mod_RRT_int)`
...
```{r pairwise_aoi1, results='asis', echo=FALSE}
